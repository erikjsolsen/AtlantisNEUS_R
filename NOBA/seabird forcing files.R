...
#' must create array with 1 time steps and reproduction scalar in step 2
#' rep_nc<-array(rmat.def, c(8,60,3))
#' rep_nc[,,2]<-rmat
rep_nc<-array(rmat.def, c(8,60,1))
rep_nc[,,1]<-rmat


#' ------------------------------- 
#' Create a list of groups_X_age_mort_ff to use to name variables in .NC file
...
for (i in 1:length(NumCohorts)){
  if (NumCohorts[i]==1) {
    GN<-as.data.frame(paste(Code[i], "mort_ff", sep=""))
    colnames(GN)<-c("GName")
    group.names<-rbind(group.names, GN)
  } else {
    ag<-c(1:NumCohorts[i])
    for (j in 1:length(ag)){
      GN<-as.data.frame(paste(Code[i],ag[j],"mort_ff", sep=""))
      colnames(GN)<-c("GName")
      group.names<-rbind(group.names, GN)
    }
  }
